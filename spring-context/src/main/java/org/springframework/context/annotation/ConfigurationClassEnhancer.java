...
public class ConfigurationClassEnhancer {
	 ...
	 *
	 * BeanFactoryAwareMethodInterceptor类的作用是对生成的代理类中 BeanFactory 属性进行赋值
	 */
	...
	public Class<?> enhance(Class<?> configClass, @Nullable ClassLoader classLoader) {
		/**
		 * 如果当前类 实现了EnhancedConfiguration接口则直接返回，说明生成的代理类会实现EnhancedConfiguration接口。
		 * 这里的作用也就是防止重复生成代理类。
		 */
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		/**
		 * 生成的代理类实现 EnhancedConfiguration 接口，作用是防止重复创建代理类
		 */
		...
		/**
		 * 将 BeanFactory 设置成生成代理类的属性
		 */
		enhancer.setStrategy(new BeanFactoryAwareGeneratorStrategy(classLoader));

		/**
		 * setCallbackFilter()方法 和 setCallback()方法 作用相似：
		 * setCallbackFilter()方法 使用的前提是需要传入 多个Callback类，其中 setCallbackFilter()方法 只是返回数组下标决定使用哪个Callback类。
		 * 区别是 setCallbackFilter()方法 会默认为所有的方法增强，而 setCallback()方法 会过滤一些方法，只有符合过滤条件的才会增强。
		 * 其中 Spring当中主要过滤@Bean方法和setBeanFactory()方法增强。
		 */
	 ...
	 *
	 * 该接口提供了 isMatch() 使得 继承的方法可以自定义是否匹配到指定的 Callback 方法
	 */
	...
	private static class ConditionalCallbackFilter implements CallbackFilter {
		...
		/**
		 * 该方法在生成CGLIB时使用，决定了使用传入的哪个 Callback 方法
		 * @param method
		 * @return
		 */
		@Override
		public int accept(Method method) {
			for (int i = 0; i < this.callbacks.length; i++) {
				...
				/**
				 * 过滤匹配传入的 method 方法，其中 isMatch() 方法有提供的Callback方法自定义实现。
				 */
